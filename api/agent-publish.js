...
export default async function handler(req,res){
  ...
  for(const k of ['GITHUB_TOKEN','GITHUB_OWNER','GITHUB_REPO'])if(!process.env[k])return json(res,503,{error:'GitHub publisher is not configured on this deployment.'});
  let body;try{body=typeof req.body==='string'?JSON.parse(req.body):req.body}catch{return json(res,400,{error:'Invalid JSON.'})}
  try{validate(body?.spec);if(body?.proof?.passed!==true)throw Error('Browser validation proof required.')}catch(e){return json(res,400,{error:e.message})}
