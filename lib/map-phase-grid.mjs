...
export function phaseTasks(phase, previousPhases, lastIndex, batch) {
  ...
  const tasks = [];
  for (let position = Math.max(0, (lastIndex ?? -1) + 1 - offset); position < count && tasks.length < batch; position++) {
    let lat, lng, dx, dy;
    if (phase.phase === 1) {
